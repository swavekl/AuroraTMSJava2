...
public class TournamentEntryInfoServiceTest extends AbstractServiceTest {
    ...
    public void testAll () {
        ...
        for (TournamentEntryInfo tournamentEntryInfo : allEntryInfosForTournament) {
            ...
            assertNotNull(tournamentEntryInfo.getFirstName(), "first name is null for player profile " + tournamentEntryInfo.getProfileId());
            assertNotNull(tournamentEntryInfo.getLastName(), "last name is null for player profile " + tournamentEntryInfo.getProfileId());
            assertNotNull(tournamentEntryInfo.getEventIds(), "events are empty for player profile " + tournamentEntryInfo.getProfileId());
        }
    }
}
